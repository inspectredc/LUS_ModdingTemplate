...
#include "../../utils/actor_utils.h"
#include "../../utils/z_scene_utils.h"


// Notes
// Skull Mask is used in place of Stone Mask
#undef REQUIRE_SFX // SFX Are Not Currently Easy To Implement Via Modding
#undef REQUIRE_ICE_BLOCK // Ice Block is a child Actor Spawned from this Actor
#undef REQUIRE_MM_CLEAR_TAG // MM Clear Tag Has Additional Modes
#undef REQUIRE_MM_PLAYER // Actor has some specific reactions to MM Player things
#undef REQUIRE_DRAW_DAMAGE_EFFECTS // MM Has a draw damage effect system
...
// static ActorDBInit En_Slime_Profile = {
//     "En_Slime",
//     "Chuchu",
//     ACTORCAT_ENEMY,
//     FLAGS,
...
void EnSlime_Idle(EnSlime* this, PlayState* play) {
    ...
    // If player is not wearing the skull mask, is within 280 units of the chuchu,
    // and the chuchu is facing the player, initialize chase/attack cycle.
    if ((Player_GetMask(play) != PLAYER_MASK_SKULL) && (this->actor.xzDistToPlayer < 280.0f) &&
...
void EnSlime_MoveInDirection(EnSlime* this, PlayState* play) {
    ...
    } else if ((Player_GetMask(play) != PLAYER_MASK_SKULL) && (this->actor.xzDistToPlayer < 280.0f) &&
               Actor_IsFacingPlayer(&this->actor, 0x5000)) {
        // If player is close enough to chuchu while not wearing the skull mask, and the chuchu is facing player,
...
void EnSlime_MoveToHome(EnSlime* this, PlayState* play) {
    ...
    this->actor.speedXZ = (0.8f * timerFactor) + 0.2f;
...
void EnSlime_ReactToBluntHit(EnSlime* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        if (Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > this->distLimit) {
            EnSlime_SetupMoveToHome(this);
        } else if ((this->actor.xzDistToPlayer < 280.0f) && (Player_GetMask(play) != PLAYER_MASK_SKULL)) {
            EnSlime_SetupTurnToPlayer(this);
        } else {
            EnSlime_SetupIdle(this);
        }
...
void EnSlime_Damaged(EnSlime* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        if (this->actor.colChkInfo.health != 0) {
            this->collider.base.acFlags |= AC_ON;
            if ((this->actor.xzDistToPlayer < 280.0f) && (Player_GetMask(play) != PLAYER_MASK_SKULL)) {
                EnSlime_SetupTurnToPlayer(this);
            } else {
                EnSlime_SetupIdle(this);
            }
...
void EnSlime_Dead(EnSlime* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->actor.scale.x, 0.0f, 0.002f)) {
        ...
        if (this->actor.params == EN_SLIME_TYPE_YELLOW) {
            Item_DropCollectible(play, &this->actor.world.pos, ITEM00_ARROWS_MEDIUM);
...
void EnSlime_Revive(EnSlime* this, PlayState* play) {
    f32 rescaleFactor1;
    f32 rescaleFactor2;
    static Vec3f gZeroVec3f = { 0.0f, 0.0f, 0.0f };
...
#ifdef REQUIRE_MM_CLEAR_TAG
                        Actor_Spawn(&play->actorCtx, play, ACTOR_EN_CLEAR_TAG, this->collider.info.bumper.hitPos.x,
                                    this->collider.info.bumper.hitPos.y, this->collider.info.bumper.hitPos.z, 0,
    ...
    if (this->iceBlockTimer == ICE_BLOCK_UNUSED) {
        ...
        if (this->drawDmgEffAlpha > 0.0f) {
            if (this->drawDmgEffType != ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX) {
                ...
                func_8002F974(thisx, NA_SE_EV_ICE_FREEZE - SFX_FLAG);
            }
        }
    }
}
...
void EnSlime_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc == EnSlime_Damaged) {
        ...
        Matrix_RotateX(this->wobbleRot.x, MTXMODE_APPLY);
        Matrix_RotateZ(this->wobbleRot.z, MTXMODE_APPLY);
    ...
    if (this->actionFunc == EnSlime_Revive) {
        ...
        Matrix_RotateY(this->reviveRotY, MTXMODE_APPLY);
...
#ifdef REQUIRE_DRAW_DAMAGE_EFFECTS
